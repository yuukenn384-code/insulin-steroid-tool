...
- **後続の追加修正（同日・iPhone実機フィードバック反映、版 2026-06-13b〜d）**：
  - **表示順**：BBIで 朝/昼/夕ボーラス→**基礎〔眠前〕は最後**に統一（提案`nextEditFields`・確定`editFields`・現在量`renderDaily`・初期`renderInit`・ログ`doseSummary`）。基礎は眠前投与のため。
  - **「食前」表示の整理**：`doseSummary` は強化インスリン（朝/昼/夕ボーラスあり）では `prandialEach`＝「食前」を**非表示**（NPH系のみ表示）。BBIで残っていた未使用値のノイズを排除。
  - **入力ボックスの段ずれ修正**：`.glucinputs` を `align-items:end`＋ラベル下寄せにし、ヒント有無で入力欄がズレないように。
  - **版表示**：ヘッダーに「版 2026-06-13d」を表示（端末キャッシュの識別用。SWは無いのでブラウザキャッシュは再読み込みで更新）。
  - **新規登録のベースライン**：`confirmRegimen` で初回（ログ空）のみ、登録の血糖プロファイル（直近24h）を**前日付（today-1）の記録**として作成（`seedBaselineFromProfile()`/`prevDateISO()` 新設）。新規患者でも前日ブロックがprefillされ day-1 から調整可、ログに1行（dose=「-」）残る。
  - **ログの定時量表記**：眠前基礎を朝昼夕と同じ並びに → 強化`朝4・昼6・夕6・眠2`／基礎のみ`眠8`（改行回避）。
- **検証**：`AI-Company/Project/インスリンツール改訂_朝入力A案/test/run.mjs`（jsdom・**24ケース**：既存15＋A案/追加9。M1=マージで前日FBSが消えない回帰、M8/M9=新規登録ベースライン）。`node --check`＋`node run.mjs`で全PASS。改訂前バックアップ：同 Project の `backup/`。
